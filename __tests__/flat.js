...
// needs a key
const Inline = ({ children, style, key }) => (
  <span key={key} style={style}>
    {children}
  </span>
);

const stylesReact = createStylesRenderer(Inline, customStyleMapReact);
...
const blocksReact = {
  unstyled: (children, { keys }) => children.map(
    (child, index) => <p key={keys[index]}>{child}</p>,
  ),
...
const correctRender = `<p><span style="${bold}">Lorem </span><a href="http://zombo.com/"><span style="${getStyles(
  bold,
  italic,
)}">ipsum</span></a><span style="${getStyles(
  bold,
  italic,
  textDecoration,
)}"> dolor</span><span style="${italic}"> sit amet,</span> pro nisl sonet ad. </p><blockquote>Eos affert numquam id, in est meis nobis. Legimus singulis suscipiantur eum in, <span style="${italic}">ceteros invenire </span>tractatos his id. </blockquote><p><span style="${bold}">Facer facilis definiebas ea pro, mei malis libris latine an. Senserit moderatius vituperata vis in.</span></p>`; // eslint-disable-line max-len
...
describe('redraft with flat styles', () => {
  ...
  test('should render flat styles correctly with ReactDOMServer.renderToStaticMarkup', () => {
